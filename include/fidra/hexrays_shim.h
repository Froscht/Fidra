...
struct ccase_t : cinsn_t {
    std::vector<uint64_t> values;
};

struct cswitch_t {
    cexpr_t* expr = nullptr;
    std::vector<ccase_t> cases;
};
...
class mba_t;
inline mba_t* decompile_func(func_t* /*pfn*/, hexrays_failure_t* hf = nullptr, int /*flags*/ = 0) {
    if (hf) { hf->code = -1; hf->desc = "Fidra: decompile_func shim"; }
    return nullptr;
}
...
// cinsn_t helpers used by traversal code
struct cif_t {
    cexpr_t* expr = nullptr;
...
struct cfor_t {
    cexpr_t* init = nullptr;
    cexpr_t* expr = nullptr;
    cexpr_t* step = nullptr;
